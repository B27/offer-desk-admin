<script>
    ...
    let imageClick = () => {fileInput.click();};
    ...
    function readUrl({target:input}){
        if(input.files && input.files[0])showLocalImg(input.files[0]);
    }
    function drag(args){
...
<input type="file" style="display:none" bind:this={fileInput} on:change={readUrl}/>
<div class="category" on:drop|preventDefault={drag}>
    <img width="64px" height="64px" src={category.image} alt="" on:click={imageClick}>
    <input 
        type="text" 
